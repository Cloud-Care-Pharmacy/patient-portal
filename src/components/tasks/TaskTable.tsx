...
import {
  ...
  isTaskCompleted,
  isTaskUnassigned,
} from "@/components/tasks/TaskActionsCell";
import {
  TaskAssigneeCell,
  formatTaskRole,
  getTaskAssigneeLabel,
  getTaskInitials,
} from "@/components/tasks/TaskAssigneeCell";
...
export function TaskTable({
    ...
    () =>
      tasks.filter((task) => {
        ...
        return matchesSearchQuery(searchQuery, [
          ...
          task.assignedRole ? formatTaskRole(task.assignedRole) : undefined,
  ...
  const filters: FilterDefinition[] = [
    ...
    },
    {
      ...
      formatOption: (option) => formatTaskRole(option as UserRole),
    },
  ];
  ...
  const allColumns: GridColDef<Task>[] = [
        ...
        return (
          ...
          >
            <span className="flex size-8 shrink-0 items-center justify-center rounded-full bg-secondary text-xs font-semibold text-secondary-foreground">
              {getTaskInitials(params.row.patientName)}
            </span>
    ...
    },
    {
      ...
      flex: 0.9,
      minWidth: 180,
      valueGetter: (_value, row) => getTaskAssigneeLabel(row),
      renderCell: (params) => {
        ...
        return <TaskAssigneeCell task={params.row} />;
      },
    },
    {
      field: "dueAt",
  ...
  if (!loading && tasks.length === 0 && !hasUserVisibleFilters) {
    return (
      <div style={{ width: "100%" }}>
        {toolbar}
        <EmptyState
  ...
  if (!loading && visibleTasks.length === 0) {
    if (!showFilterControls && !hasSearchQuery) {
      return (
        <div style={{ width: "100%" }}>
          {toolbar}
          <EmptyState
